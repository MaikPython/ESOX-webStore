 ...
 */
const checkIfNumber = (inputString) =>{
    return !isNaN(inputString)
}

const isNumberValid = (inputString, {precision = 9, scale = 0}) => {
    const parts = inputString.split('.')
    let inputBeforeComa = parts[0]
    let inputAfterComa = parts[1] 

    const scaleCount = inputAfterComa ? inputAfterComa.length:0
    const digitCount = inputBeforeComa.length + scaleCount

    checkIfNumber(inputString)
    
    return(digitCount <= precision && scaleCount <= scale)
